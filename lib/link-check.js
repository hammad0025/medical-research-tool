...
export const assertSafeProbeUrl = async (rawUrl, { resolver = lookup } = {}) => {
  let parsed;
  try {
  ...
  const attempt = async (method, extraHeaders) => {
    ...
    try {
      ...
      for (let redirects = 0; redirects <= 5; redirects++) {
        await assertSafeProbeUrl(current, { resolver });
        ...
        if (r.status < 300 || r.status > 399) return { ok: true, status: r.status };
        const location = r.headers?.get?.('location');
        if (!location) return { ok: true, status: r.status };
