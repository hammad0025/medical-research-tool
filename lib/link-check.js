...
// `blockedIsDead: false` narrows the contract to "definitively gone" (404/410).
// It exists for links that are a RESOURCE rather than a claim citation — a
// patient-advocacy or centre homepage. The strict rule is right for a citation:
// a 403 there means the reader cannot reach the evidence behind a medical
// claim, so the claim must not stand on it. An organisation's front door is
// different — nothing is being evidenced, and a datacenter IP being bounced by
// bot protection says nothing about whether a person can open it. ibpf.org
// answers 200 to a browser and failed this audit nightly from CI's runner.
export const classifyProbeStatus = (url, status, { blockedIsDead = true } = {}) => {
  if (status === 404 || status === 410) return 'dead';
  if (status === 401 || status === 403 || status === 451) {
    if (!blockedIsDead) return 'alive';
...
export const probeUrl = async (url, timeoutMs, {
  ...
  const verdict = (status) => classifyProbeStatus(url, status, { blockedIsDead });
  ...
  if (res.ok) {
    if (verdict(res.status) === 'alive') {
      return { url, finalUrl: res.finalUrl || url, dead: false, status: res.status };
    }
    ...
    if (confirm.ok) {
      return {
        ...
        dead: verdict(confirm.status) === 'dead',
        status: confirm.status
      };
    }
    if (confirm.aborted) return { url, dead: false, reason: 'timeout' }; // transient → keep
    // A connection-level failure is only evidence of absence for a citation we
    // must be able to stand a claim on; for a resource link it is as likely to
    // be bot protection dropping the handshake.
    return { url, dead: blockedIsDead, reason: 'network' };
  }
  if (res.aborted) return { url, dead: false, reason: 'timeout' };  // transient → keep
...
export const findDeadLinks = async (urls, {
  ...
  // Narrow the contract to "definitively gone" — see classifyProbeStatus.
  blockedIsDead = true,
  // Optional out-param: url → { status, reason, dead }, so a caller can report
  // WHY a link was judged dead. "DEAD https://ibpf.org" with no status sent me
  // to curl to find out the site was up and CI's IP was being bounced.
  details = null
} = {}) => {
  const dead = new Set();
  ...
  const worker = async () => {
    while (idx < toCheck.length && Date.now() < deadline) {
      ...
      const result = await probeUrl(url, timeoutMs, { blockedIsDead });
      if (details) {
        details.set(url, { status: result.status ?? 0, reason: result.reason || 'status', dead: !!result.dead });
      }
      if (result.dead) dead.add(url);
    }
  };
  await Promise.all(
