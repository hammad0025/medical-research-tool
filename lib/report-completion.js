...
import { REPURPOSE_SECTION_NEVER, resolveRepurposeSection } from './repurpose-quality.js';
import { stableJson } from './stable-serialize.js';

const REPORT_CONTRACT_VERSION = '2026-07-19.1';
...
const reportCompletionMessage = (code) =>
...
// resolveRepurposeSection is the single shared classifier (also used by the
// client and by regression scripts) so this can never drift from the
// client's behavior the way two independent implementations already did.
const hasLinkedRepurposeCandidate = (text) =>
  String(text || '').split(/(?=^CANDIDATE:\s*)/gim).some((block) =>
    /^CANDIDATE:\s*\S+/im.test(block) &&
    (
      /\[[^\]]+\]\(https?:\/\/[^)]+\)/i.test(block) ||
      resolveRepurposeSection(block) === REPURPOSE_SECTION_NEVER
    )
  );
...
const reportContentDigest = (value) =>
