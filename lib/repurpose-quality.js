...
export const REPURPOSE_BACKFILL_MAX_PASSES = 3;
...
//   2. DailyMed search/drugInfo for a drug,
//   3. Otherwise any http(s) URL that is NOT a Google search.
// Google-search-only MUST NEVER count as a paper citation for Hard 25.
// Dead/invented deep links are stripped earlier by sanitize + removeDeadLinks;
// what remains here after sanitization is treated as real enough for the count.
// ---------------------------------------------------------------------------

export const isGoogleSearchUrl = (url) =>
  /^https?:\/\/(www\.)?google\.[a-z.]+\/search\b/i.test(String(url || ''));

export const isDailyMedUrl = (url) =>
  /^https?:\/\/(www\.)?dailymed\.nlm\.nih\.gov\/dailymed\/(search|drugInfo)\b/i.test(
    String(url || '')
  );
...
/** DailyMed search URL for a specific drug name (registry-fill citations). */
 ...
 * Google search never qualifies. Empty/non-http never qualifies.
 */
 ...
 * in references or supporting_evidence (Google alone does not count).
 */
export const candidateHasRealCitation = (candidate = {}) => {
  const blob = `${candidate.references || ''} ${candidate.supporting_evidence || ''}`;
  return extractCitationUrls(blob).some(isRealCitationUrl);
};

export const textHasRealCitation = (blockText) =>
  extractCitationUrls(blockText).some(isRealCitationUrl);
 ...
 */
export const distinctLinkedCandidateCount = (text) => {
  const keys = new Set();
  for (const block of splitCandidateBlocks(text)) {
    if (!textHasRealCitation(block)) continue;
 ...
 */
export const buildRegistryFillCandidate = (drug, { condition = '' } = {}) => {
  ...
  const dm = dailyMedSearchUrl(name);
  const why = mechanism
...
SUPPORTING_EVIDENCE: No human study for ${cond} was available in the grounded pack for this run. The only source attached is the FDA DailyMed search for this drug name — not a fabricated paper.
REFERENCES: [DailyMed label search — ${name}](${dm})
...
export const appendRegistryFill = (
  ...
  for (const drug of pool) {
    ...
    const block = buildRegistryFillCandidate(
      typeof drug === 'string' ? { name } : drug,
      { condition }
    );
