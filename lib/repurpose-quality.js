...
export const REPURPOSE_SECTION_TARGET = 25;
// Hard floor on the FINAL curated list: ~25 per section ≈ 50 linked cards.
// Every displayed card should carry a REAL citation when available (pack /
// CT.gov NCT / DailyMed). Quantity is still gated by quality — registry fill
// is never-researched + DailyMed only, never fake papers.
export const REPURPOSE_TARGET_TOTAL = 50;
export const REPURPOSE_SOFT_CAP = 50;
/** @deprecated Prefer REPURPOSE_TARGET_TOTAL; kept for callers that still say "min". */
export const REPURPOSE_MIN_TOTAL = 50;
/**
 ...
 */
export const REPURPOSE_BACKFILL_THRESHOLD = 50;
...
export const buildRegistryFillCandidate = (drug, { condition = '', labelUrl = '' } = {}) => {
  ...
  const why = mechanism
    ? `• There is no research on this specific drug for ${cond}. However, our research suggests it may help, and here is why: it is an FDA-tracked medicine (${mechanism}) with pathway overlap. Discuss this with your doctor.\n• No published trial for ${cond} was available in this run — labeled MECHANISTIC_ONLY on purpose.\n• This is an educational idea only, not a recommendation.`
    : `• There is no research on this specific drug for ${cond}. However, our research suggests it may help based on pathway overlap with how the disease works. Discuss this with your doctor.\n• No published trial for ${cond} was available in this run — labeled MECHANISTIC_ONLY on purpose.\n• This is an educational idea only, not a recommendation.`;
...
WHY_FOR_THIS_CONDITION: There is no research on this specific drug for ${cond}. However, our research suggests it may help based on pathway overlap. Discuss this with your doctor.
...
SUPPORTING_EVIDENCE: No human study for ${cond} was available in the sources gathered for this run — and a DailyMed search page is not a source, so no citation link is fabricated.
${referencesLine}
