...
export const REPURPOSE_RESEARCHED_LANE = 1;
/** Lane index whose entire output is condition-specific mechanistic ideas. */
export const REPURPOSE_MECHANISTIC_LANE = 2;
...
/**
 * A novel gene therapy, cell therapy or sponsor programme code is a pipeline
 * programme, not a drug or supplement a patient can raise with their clinician.
 * Those have their own section of the report; listing them again as "ideas"
 * spends a slot and repeats what the reader already saw. Validated against the
 * live candidate names — it drops jCell, OCU400, MCO-010, EDIT-101 and AAV
 * constructs while keeping CoQ10, vitamin B12, TUDCA, NACA and CAI therapy.
 */
const PIPELINE_PROGRAMME = /\b(?:aav\d*|crispr|cas9|gene therapy|gene[- ]edit|stem cell|progenitor|optogenetic|hesc|ipsc|allorx|umsc|antisense oligonucleotide|[a-z]+vec)\b/i;
// Sponsor codes come in several shapes and the digit run is not bounded:
// CC-90001, BMS-986278 and BI 1015550 all escaped a \d{2,4} pattern.
const PROGRAMME_CODE_NAME = /^[A-Z]{2,5}[-\s]?[A-Z]{0,2}\d{2,}/;
// Registry arm labels that name no agent at all.
const NOT_AN_AGENT_LABEL = /^(?:treatment as usual|usual (?:care|treatment)|standard (?:of )?care|best supportive care|no intervention|placebo|sham|control(?: arm)?|test drug.*|study drug.*)$/i;

export const isPipelineProgramme = (name) => {
  const n = String(name || '').trim();
  if (!n) return false;
  return PIPELINE_PROGRAMME.test(n) || PROGRAMME_CODE_NAME.test(n) || NOT_AN_AGENT_LABEL.test(n);
};
...
// An FDA label documents an agent's APPROVED use, which is by definition a
// different condition here. A candidate whose only citation is a label has not
// been shown to be researched for this condition, so it belongs in the
// no-study section with its honest disclaimer rather than in a section that
// tells the reader condition-specific research exists.
const hasNonLabelCitation = (blob) => {
  const urls = String(blob || '').match(/https?:\/\/[^\s)\]"']+/g) || [];
  if (!urls.length) return false;
  return urls.some((u) => !isDailyMedLabelUrl(u) && !isDailyMedSearchUrl(u) &&
    !isGoogleSearchUrl(u) && !/accessdata\.fda\.gov|dailymed\.nlm\.nih\.gov/i.test(u));
};
...
export const resolveRepurposeSection = (blockOrFields = {}, opts = {}) => {
  ...
  if (tagged.includes('researched')) {
    // Claiming condition-specific research requires a source that is not just
    // the agent's approved-use label.
    return hasNonLabelCitation(blob) || opts.hasCitation === undefined
