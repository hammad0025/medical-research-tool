...
export const getInfraStatus = () => {
  ...
  if (alertsEmail.ready) ok.push('alerts_email');
    ...
    why: 'Required for production alert delivery with a verified sender and public links.'
  });
  if (isAlertMonitoringConfigured()) ok.push('alerts_monitoring');
  else missing.push({
    id: 'ALERTS_MONITOR_WEBHOOK_URL',
    why: 'Required so alert delivery failures notify an operational monitor.'
  });
