...
// Pipeline drugs cite their supporting paper by id (`evidenceRef`) instead of
// carrying a link inline. The Pipeline Watch table's link gate checks the drug
// object for nct/url/pmid, so an unresolved ref left the row out entirely — for
// most curated conditions the whole structured table was empty even though the
// referenced item had a real url/pmid/doi. Resolve each ref to its item's link
// so a curated pipeline drug with a genuine reference actually shows up. Drugs
// that already carry a link, or whose ref does not resolve, pass through
// unchanged (never fabricate a link).
export const resolvePipelineDrugEvidenceLinks = (pipelineDrugs, itemsById) => {
  if (!Array.isArray(pipelineDrugs)) return [];
  const byId = itemsById instanceof Map ? itemsById : new Map();
  return pipelineDrugs.map((drug) => {
    if (!drug || typeof drug !== 'object') return drug;
    if (drug.nct || drug.url || drug.pmid) return drug;
    const item = drug.evidenceRef != null ? byId.get(drug.evidenceRef) : null;
    if (!item) return drug;
    const link = {};
    if (item.url) link.url = item.url;
    else if (item.doi) link.url = `https://doi.org/${item.doi}`;
    if (item.pmid) link.pmid = item.pmid;
    if (!link.url && !link.pmid) return drug;
    return { ...drug, ...link };
  });
};
...
const kbRecordToLoadResult = (kb, match) => {
  ...
  return {
    ...
    meta: {
      ...
      pipelineDrugs: resolvePipelineDrugEvidenceLinks(kb.pipelineDrugs, activeItemsById),
