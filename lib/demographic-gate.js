...
const FEMALE_ONLY_TEXT =
  /\bfemale[- ]pattern\b|\bwomen[- ]only\b|\bfemale[- ]only\b|\bin women\b|\bwomen with\b|\bpost-?menopaus|\bpre-?menopaus|\bpregnan|\bfemales? only\b/i;
const MALE_ONLY_TEXT =
  /\bmale[- ]pattern\b|\bmen[- ]only\b|\bmale[- ]only\b|\bin men\b|\bmen with\b|\bprostat|\bmales? only\b/i;
...
const ADULT_ONLY_TEXT =
  /\badults? only\b|\bin adults\b|\badult[- ]only\b|\b(?:18|21) years and older\b/i;
...
export const textWrongAgeBand = (text, patientAge) => {
  ...
  const t = String(text || '');
  if (age >= 18 && PEDIATRIC_ONLY_TEXT.test(t) && !ADULT_ONLY_TEXT.test(t)) return true;
  if (age < 18 && ADULT_ONLY_TEXT.test(t) && !PEDIATRIC_ONLY_TEXT.test(t)) return true;
  return false;
};
...
export const findingIsDemographicMismatch = (quote, patient = {}) => {
  ...
  const sex = normalizePatientSex(patient?.gender ?? patient?.sex);
  const age = parsePatientAge(patient?.age);
  return textOppositeSexOnly(t, sex) || textWrongAgeBand(t, age) || lineMentionsStudy(t);
};
