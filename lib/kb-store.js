...
import { timeoutFor, withTimeout } from './fetch-timeout.js';
...
const redisCall = (operation, label) => withTimeout(operation, {
  timeoutMs: REDIS_TIMEOUT_MS,
  provider: `Dynamic KB Redis ${label}`
});
...
export const getDynamicKb = async (slug) => {
  if (!slug) return null;
  if (useUpstash) {
    const kb = await redisCall(() => redis.get(`${PREFIX}${slug}`), 'get');
    if (typeof kb === 'string') {
...
export const lookupDynamicKb = async (conditionText) => {
  ...
  if (useUpstash) {
    const slug = await redisCall(() => redis.get(`${ALIAS_PREFIX}${key}`), 'alias lookup');
    if (slug) {
      const kb = await getDynamicKb(String(slug));
      if (kb) return kb;
      // Repair a dangling alias left by data created before atomic writes.
      await redisCall(() => redis.del(`${ALIAS_PREFIX}${key}`), 'alias repair');
    }
...
export const listDynamicKbSlugs = async () => {
  if (useUpstash) {
    return (await redisCall(() => redis.smembers(INDEX_KEY), 'index list')) || [];
  }
  return [...mem.index];
};
...
export const repairDynamicKbStore = async () => {
  ...
  for (const slug of slugs) {
    ...
    if (useUpstash) await redisCall(() => redis.srem(INDEX_KEY, slug), 'index repair');
