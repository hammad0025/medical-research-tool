import { clampText, citationLabel } from './card-identity.js';
  ...
  for (const item of kbItems) {
    ...
    for (const part of splitConjoinedAgents(name)) {
      ...
      seeds.push({
        ...
        category,
        title: item.title,
        // Was a bare 150-char slice: a curated TUDCA summary reached the page
        // ending "...used for centuries in Chinese medicin".
        summary: clampText(item.summary || item.text || '', 220)
      });
    }
  }
  return seeds;
...
export const renderResearchedCandidateBlocks = (seeds = []) => seeds
  ...
  .map((s) => {
    ...
    return [
      ...
      `SUPPORTING_EVIDENCE: ${finding} ([${citationLabel(s)}](${s.url}))`,
      `REFERENCES: [${citationLabel(s)}](${s.url})`,
      ''
    ].join('\n');
  })
  .join('\n');
