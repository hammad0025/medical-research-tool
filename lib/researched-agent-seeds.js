...
export const buildResearchedAgentBlock = (
  ...
  const seeds = [...curated, ...fromWeb, ...fromTrials].slice(0, 10);
...
export const renderResearchedCandidateBlocks = (seeds = []) => seeds
  ...
  .map((s) => {
    const strength = s.strength || (s.category === 'trial'
      ? 'OBSERVATIONAL'
      : (TIER_WORDS[String(s.category).toLowerCase()] || 'PRECLINICAL'));
    const finding = String(s.summary || s.title || '').replace(/\s+/g, ' ').trim()
    ...
    return [
      ...
      `SUPPORTING_EVIDENCE: ${finding} ([source](${s.url}))`,
      `REFERENCES: [${String(s.title || s.name).slice(0, 90)}](${s.url})`,
      ''
    ].join('\n');
  })
  .join('\n');
...
export const researchedAgentSeedList = (
  ...
  return [...curated, ...fromWeb, ...fromTrials].slice(0, 10);
};
