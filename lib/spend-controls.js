...
export const paidRequestControlPlan = ({
  mode,
  phase,
  pipelineMode,
  usageBypassed = false
} = {}) => {
  const splitSynthesis = Boolean(
    pipelineMode &&
    phase === 'synthesize' &&
    ['research', 'repurpose'].includes(String(mode || ''))
  );
  const meterUsage = isPaidUserMode(mode) && !splitSynthesis && !usageBypassed;
  return {
    splitSynthesis,
    meterUsage,
    requireIdempotencyAndBudget: meterUsage || splitSynthesis
  };
};
