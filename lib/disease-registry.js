// Disease registry — ~19k drug-targetable conditions (Mondo ontology, CC-BY-4.0).
// Built by scripts/build-disease-registry.mjs from everycure/disease-list export.
...
const scorePhrase = (needle, hay) => {
  ...
  if (minLen <= 3) {
    if (hasWholeWord(needle, hay) || hasWholeWord(hay, needle)) return 88;
    return 0;
  }
...
export const loadDiseaseRegistry = async () => {
  ...
  try {
    ...
    console.warn('[disease-registry] not loaded:', safeErrorMessage(e));
...
// Well-known acronyms missing from ontology synonym lists.
const ACRONYM_SLUGS = {
  lada: 'latent-autoimmune-diabetes-in-adults'
};

const findBySlug = (diseases, slug) => diseases.find((d) => d.slug === slug) || null;
...
export const lookupDisease = async (conditionText) => {
  ...
  // Bare "RP" is the ophthalmology acronym — registry only has numbered
  // subtypes (RP7, RP26, …). Let curated KB + gene-notation handle it.
  if (needle === 'rp') return null;

  const acronymSlug = ACRONYM_SLUGS[needle];
  if (acronymSlug) {
    const entry = findBySlug(registry.diseases || [], acronymSlug);
    if (entry) return { entry, score: 100, matchedOn: conditionText };
  }
...
export const buildDossierFromRegistry = (input, match) => {
  ...
  return {
    ...
    meshTerms: [],
    subspecialty,
    registrySpecialty: entry.specialty || '',
    registryGroup: entry.group || '',
    topCenters: [],
    keyInvestigators: [],
    patientAdvocacy: [],
    landmarkTrials: [],
    ...
    notes: entry.definition
      ? `${entry.definition} (Mondo · ${entry.mondo}${entry.orpha ? ` · ORPHA:${entry.orpha}` : ''})`
      : `Resolved from disease registry (${entry.mondo}${entry.orpha ? ` · ORPHA:${entry.orpha}` : ''}).`,
    mondoId: entry.mondo,
