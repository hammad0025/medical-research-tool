  ...
  if (useUpstash) {
    ...
    await upstash([
      ...
      `redis.call('SET', KEYS[1], ARGV[1])
       if ARGV[1] ~= 'free' then redis.call('SET', KEYS[2], '1') end
       ...
       return 1`,
      '3', `usage:plan-ip:${key}`, `usage:paid-ip:${key}`, usageKey,
      tier, String(45 * 24 * 3600), String(PLAN_LIMITS[tier])
    ]);
  } else {
...
export const consumeResearchCredit = async (ip) => {
  ...
  if (useUpstash) {
    const result = await upstash([
      ...
      '1', `usage:${month}:${key}`, String(limit), tier, String(45 * 24 * 3600)
    ]);
