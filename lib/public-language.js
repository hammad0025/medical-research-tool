...
// Leaking a credential or the system prompt is a security failure and must
// still abort. Leaking an internal word like "cacheHit" is a cosmetic failure,
// and aborting for it destroys a report that took minutes to produce. These
// are the matches worth failing the whole request for.
const SECRET_MATCH = new RegExp([
  'system prompt', 'developer message', 'hidden instruction', 'chain[\\s-]*of[\\s-]*thought',
  'private orchestration', 'internal[\\s-]*only instruction',
  'API[\\s_-]*KEY', 'authorization\\s*:\\s*bearer', 'access token', 'secret token',
  'stack trace', 'SSRF', 'cross[\\s-]*site scripting', 'XSS'
].join('|'), 'i');

// Remove an internal term wherever it appears, tolerating the punctuation and
// spacing variants the skeleton matcher detects it through.
const redactTerm = (text, term) => {
  const chars = String(term).split('').filter(Boolean)
    .map((c) => c.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
  if (!chars.length) return text;
  const loose = new RegExp(chars.join('[^\\p{L}\\p{N}]{0,3}'), 'giu');
  return redactTermBySkeleton(text.replace(loose, 'internal detail'), term);
};

/**
 * Remove a term the way the DETECTOR sees it.
 *
 * Detection collapses text to an alphanumeric skeleton, so "profile - * - Key"
 * reads as one term. The regex above only tolerates a few punctuation
 * characters between letters, so anything split more widely was detectable but
 * not removable — and the caller then threw, discarding a finished report over
 * the word "profileKey" split across a bullet.
 *
 * Mapping each skeleton character back to its offset in the raw text closes
 * that gap: whatever detection can find, this can delete.
 */
const redactTermBySkeleton = (text, term) => {
  const needle = publicLanguageSkeleton(term);
  if (!needle) return text;
  const raw = String(text);
  // Rebuild the detector's skeleton character by character, remembering which
  // raw offset each skeleton character came from. This has to mirror
  // publicLanguageSkeleton exactly — NFKC, zero-width strip, lowercase,
  // confusable folding — or an obfuscated term ("evіdence pack" with a Cyrillic
  // i, joined by U+2060) stays detectable and un-redactable, which is the same
  // asymmetry one layer down.
  let skeleton = '';
  const offsets = [];
  for (let i = 0; i < raw.length; i += 1) {
    const folded = canonicalizePublicUnicode(raw[i]).toLowerCase();
    for (const char of folded) {
      const ascii = CONFUSABLE_ASCII[char] || char;
      if (/[\p{L}\p{N}]/u.test(ascii)) {
        skeleton += ascii;
        offsets.push(i);
      }
    }
  ...
  let from = 0;
  for (;;) {
    const hit = skeleton.indexOf(needle, from);
    if (hit === -1) break;
    spans.push([offsets[hit], offsets[hit + needle.length - 1] + 1]);
  ...
  for (const [start, end] of spans) {
    out += raw.slice(cursor, start) + 'internal detail';
    cursor = end;
  }
...
const assertPublicLanguage = (value, options) => {
  ...
  const secrets = matches.filter((m) => SECRET_MATCH.test(String(m)));
  if (secrets.length) throw new PublicLanguageBoundaryError(secrets);
  // Cosmetic leak: redact it and let the report through. Failing here is how a
  // single unmapped word ("sourceSHA", "cacheHit") took down an entire report.
  let cleaned = String(value);
  for (const match of matches) cleaned = redactTerm(cleaned, match);
  const remaining = findProhibitedPublicLanguage(cleaned, options);
  if (remaining.length) {
    // A secret that survives redaction still has to abort — serving it is worse
    // than serving nothing.
    const stillSecret = remaining.filter((m) => SECRET_MATCH.test(String(m)));
    if (stillSecret.length) throw new PublicLanguageBoundaryError(stillSecret);
    // Otherwise let it through. Detection collapses text to a skeleton, so it
    // sees "profile - * - Key" as one term, while redactTerm works on the raw
    // text and tolerates only a few punctuation characters between letters.
    // Anything split more widely than that is detectable but not removable, and
    // throwing here discarded an entire finished report over the word
    // "profileKey" split across a bullet. That is the exact failure the branch
    // above was written to prevent, reintroduced one line later.
    console.warn(
      '[public-language] redaction incomplete:', remaining.length,
      'cosmetic term(s) remain — passing the report through rather than failing it'
    );
    return cleaned;
  }
  console.warn('[public-language] redacted internal term(s) rather than failing the report');
  return cleaned;
};
