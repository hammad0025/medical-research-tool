...
const redactTerm = (text, term) => {
  ...
  return text.replace(loose, 'internal detail');
};
...
const assertPublicLanguage = (value, options) => {
  ...
  if (remaining.length) {
    console.warn('[public-language] redaction incomplete:', remaining.length, 'term(s) remain');
    throw new PublicLanguageBoundaryError(remaining);
  }
