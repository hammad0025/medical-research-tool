...
// A SPECIFIC DailyMed label monograph from an SPL setid — the ONLY acceptable
// DailyMed citation form (a search.cfm?query=… page is a results list, not a
// label, and is banned as a citation). openFDA labels expose openfda.spl_set_id,
// which is exactly the DailyMed setid, so we can build the concrete monograph
// URL deterministically for any drug whose label we already fetched.
