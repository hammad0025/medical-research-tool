...
import {
  labelMatchesRequestedProduct,
  labelRecordFromOpenFda,
  sealFdaLabel,
...
export const selectBestLabel = (labels, { drug = '', route = '', formulation = '' } = {}) => {
  ...
  const scored = values(labels).map((label, index) => {
    ...
    const nameMatch = labelMatchesRequestedProduct(labelRecordFromOpenFda(label, {
      requestedDrug: drug
    }), drug);
  ...
  const compatible = scored.filter((entry) => !entry.mismatch && entry.score >= 4);
  ...
  try {
    ...
    if (drug) {
      ...
      try {
        ...
        if (lbl) {
          const label = labelRecordFromOpenFda(lbl, { requestedDrug: drug });
          const identityIssues = validateLabelIdentity(label, { requestedDrug: drug });
          if (!identityIssues.length) {
            ...
            providerErrors.push({
              ...
              reason: identityIssues[0].toLowerCase().replaceAll('_', '-')
            });
          }
        }
      } catch (error) {
