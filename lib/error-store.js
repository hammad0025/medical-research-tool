...
const PREFIX = 'errors:';
const ERROR_RETENTION_SECONDS = RETENTION_SECONDS.errorMemory;
...
const readList = async (key) => {
  const cutoff = Date.now() - ERROR_RETENTION_SECONDS * 1000;
  if (useUpstash) {
    const list = await redis.get(`${PREFIX}${key}`);
    return Array.isArray(list) ? list.filter((record) => Number(record?.ts) >= cutoff) : [];
  }
  const retained = mem.get(key)
    ? [...mem.get(key)].filter((record) => Number(record?.ts) >= cutoff)
    : [];
  if (retained.length) mem.set(key, retained);
  else mem.delete(key);
  return retained;
};
...
const writeList = async (key, list) => {
  if (useUpstash) {
    await redis.set(`${PREFIX}${key}`, list, { ex: ERROR_RETENTION_SECONDS });
  } else {
    mem.set(key, list);
  }
};

export const errorMemoryRetentionSeconds = () => ERROR_RETENTION_SECONDS;
...
export const recordConditionErrors = async (condition, records, { authorization = null } = {}) => {
  ...
  const containsUserWrite = incoming.some((record) => record.source === 'user');
  if (containsUserWrite && authorization?.purpose !== 'error-memory.write') {
    const error = new Error('Reviewer authorization is required for global error-memory writes');
    error.code = 'REVIEWER_AUTH_REQUIRED';
    throw error;
  }

  await appendSecurityAudit({
    action: 'error-memory.write',
    actor: containsUserWrite ? authorization.actor : 'system:validator',
    target: key
  });
