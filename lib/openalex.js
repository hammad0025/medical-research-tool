...
const OPENALEX_TIMEOUT_MS = timeoutFor('openalex', 12_000);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const sp = new URLSearchParams({
      search: String(query),
      per_page: String(Math.min(Number(limit) || 25, 50)),
    ...
    const r = await fetchWithTimeout(`${API}?${sp.toString()}`, {
      signal: req.signal
    }, {
    ...
    logError('[openalex] request failed', e);
