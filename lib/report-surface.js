...
const trialBlock = (trials = {}) => {
  ...
  const rows = studies.slice(0, 50).map((study) => [
...
const normalizeTrialStudy = (value) => {
  ...
  return {
    ...
    // api/trials.js returns contacts as an object
    // ({ centralContacts, overallOfficials, locations }), so the array-only
    // branch silently replaced it with [] for every study and any consumer
    // wanting locations or study-team contact info found nothing.
    contacts: Array.isArray(study.contacts)
      ? study.contacts.filter(isRecord)
      : (isRecord(study.contacts)
        ? {
          centralContacts: Array.isArray(study.contacts.centralContacts)
            ? study.contacts.centralContacts.filter(isRecord) : [],
          overallOfficials: Array.isArray(study.contacts.overallOfficials)
            ? study.contacts.overallOfficials.filter(isRecord) : [],
          locations: Array.isArray(study.contacts.locations)
            ? study.contacts.locations.filter(isRecord) : []
        }
        : []),
