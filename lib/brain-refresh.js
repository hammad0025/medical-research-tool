...
const fetchTrialHighlights = async (condition, limit = 8) => {
  try {
    ...
    const r = await fetchWithTimeout(`${CT_API}?${params}`, {
      headers: { Accept: 'application/json', 'User-Agent': 'medical-research-assistant/1.0' }
    }, {
      timeoutMs: CLINICAL_TRIALS_TIMEOUT_MS,
      provider: 'ClinicalTrials.gov brain refresh'
    });
    if (!r.ok) {
      return { items: [], degraded: { provider: 'ClinicalTrials.gov', reason: `http-${r.status}` } };
    }
    ...
    console.warn('[brain-refresh] trials fetch failed:', safeErrorMessage(e));
    return {
      items: [],
      degraded: {
        provider: 'ClinicalTrials.gov',
        reason: isTimeoutError(e) ? 'timeout' : 'network-error'
      }
    };
  }
};
...
async function refreshBrainForCondition(canonical, slug, { force = false } = {}) {
  ...
  const [papers, webHints, trialResult] = await Promise.all([
    gatherPubMedPapers(name, { mode: 'fast' }),
    perplexityHints(name),
    fetchTrialHighlights(name)
  ]);
  const trialHighlights = trialResult.items;
  ...
  if (hasStaticFile && staticMatch?.kb) {
    const overlay = {
      ...
      trialHighlights,
      ...(trialResult.degraded ? { providerErrors: [trialResult.degraded], degraded: true } : {}),
  ...
  if (trialResult.degraded) {
    kb.degraded = true;
    kb.providerErrors = [...(kb.providerErrors || []), trialResult.degraded];
  }
    ...
    try {
      ...
      if (entry.leaseToken) {
        try {
          ...
          console.warn('[brain-refresh] failed to schedule retry:', safeErrorMessage(retryError));
        }
      }
      results.push({
        ...publicEntry,
