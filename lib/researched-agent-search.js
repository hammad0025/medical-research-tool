...
// These searches ask for up to 24 agents with a mechanism sentence each, over
// live web results — far more work than the freshness lookup this default was
// sized for. At 15s the mechanistic search timed out on essentially every
// report, returned nothing, and the not-studied section rendered zero
// candidates. It fails open, so the timeout was silent.
const TIMEOUT_MS = Number(process.env.PERPLEXITY_DISCOVERY_TIMEOUT_MS || 45_000);
...
{"name": string (the agent, in the plain wording a patient would use), "url": string (a real resolvable URL to the study), "title": string (the exact published title of the study — NOT a sentence describing it; omit if you do not know it), "year": number, "evidence": one of "rct"|"observational"|"preclinical"|"case-report", "finding": string (one factual sentence on what the study found, including if it found no benefit)}
...
// A hard character slice cut words in half: a Parkinson card ended
// "...neuroinflammatory signaling that GLP-1 pathway activa". Prefer the last
// complete sentence inside the limit, else the last whole word, and mark the
// cut so a shortened line never reads as a finished one.
export const clampToWord = (value, limit) => {
  const text = String(value || '').replace(/\s+/g, ' ').trim();
  if (text.length <= limit) return text;
  const window = text.slice(0, limit);
  const sentence = window.match(/^[\s\S]*[.!?](?=\s|$)/);
  if (sentence && sentence[0].trim().length >= limit * 0.5) return sentence[0].trim();
  const cut = window.slice(0, window.lastIndexOf(' '));
  return `${(cut || window).replace(/[\s,;:—–-]+$/, '')}\u2026`;
};
  ...
  try {
    ...
    return items
      ...
      .map((item) => ({
        ...
        // `title` must stay a real study title. Assigning the finding sentence
        // to it meant every downstream consumer that treats `title` as a
        // citation label rendered a rationale sentence as the link text, cut
        // mid-word. Empty is the honest value when the provider gives none —
        // the seed renderer then falls back to the agent's own name.
        title: clampToWord(item.title || '', 200),
        summary: clampToWord(item.finding || '', 300)
      }));
  } catch (error) {
...
Return a STRICT JSON array of at most 24 objects, each:
{"name": string (plain wording a patient would use), "mechanism": string, "usualUse": string (what it is normally prescribed or taken for), "url": string (a real resolvable URL supporting the MECHANISM or the agent's usual use)}

"mechanism" is read by a patient, not a clinician. Write ONE short sentence at a
7th-grade reading level: what goes wrong in this condition, and what this agent
does about it. Use everyday words. If a technical term is unavoidable, put a
plain-English gloss right after it in parentheses. Do not open with the agent's
drug class or a pathway name.
  GOOD: "In this disease the light-sensing cells die when they are starved of
         energy; this drug helps cells clear out damaged parts and keep working."
  BAD:  "Photoreceptor degeneration involves impaired autophagy and pro-death
         mTOR signaling, so mTOR inhibition is mechanistically relevant."
...
- Propose generously. Anything already studied in this condition is filtered out afterwards, so a short list simply leaves the reader with fewer ideas.
- No duplicates. Output ONLY the JSON array.`;
};
...
const attemptMechanisticSearch = async ({ condition, exclude, signal }) => {
  ...
  try {
    ...
    return items
      ...
      .map((item) => ({
        name: String(item.name).slice(0, 60).trim(),
        mechanism: clampToWord(item.mechanism, 240),
        usualUse: clampToWord(item.usualUse, 120),
