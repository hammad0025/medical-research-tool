...
export async function getDossier(condition, { bypassCache = false, signal = null } = {}) {
  ...
  try {
    ...
    console.warn('[disease-dossier] registry short-circuit failed, falling back to LLM:', safeErrorMessage(err));
  }
  ...
  const work = (async () => {
    const raw = await callDossierAgent(input, apiKey, signal);
