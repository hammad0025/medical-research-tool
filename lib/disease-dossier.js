...
import { clampText } from './card-identity.js';
...
const sanitizeDossier = (raw, inputCondition) => {
  ...
  return {
    ...
    // Was a bare 400-char slice, which shipped a safety flag ending
    // "...path for RP is through ClinicalT" to the reader.
    redFlags: arr(r.redFlags, 8).map((s) => clampText(s, 400)).filter(Boolean),
