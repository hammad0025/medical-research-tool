...
export async function getDossier(condition, { bypassCache = false } = {}) {
  ...
  if (!bypassCache) {
    ...
    if (hit && Date.now() - hit.cachedAt < DOSSIER_CACHE_TTL_MS) {
      return { ...hit.dossier, cacheHit: true };
    }
  }
  ...
  if (!apiKey) {
    ...
    return { ...fb, cacheHit: false, fallbackReason: 'no_api_key' };
  }

  const raw = await callDossierAgent(input, apiKey);
  // If the agent errored, use the fallback but attach the error reason so
  // downstream (chat / research prompt, UI) can say WHY the dossier is empty
  ...
  // fallback too, which meant one transient Anthropic timeout/429 on the
  // first request permanently (for 24h) pinned this condition to an
  // "agent was unavailable" empty dossier even though retrying would
  // have worked. The user saw this as "I ran research 4 times and it
  // kept failing with the same error." Don't cache failures — let each
  // request get a fresh attempt until one succeeds.
  if (!errored) {
    dossierCache.set(key, { dossier, cachedAt: Date.now() });
  }
  return { ...dossier, cacheHit: false };
}
