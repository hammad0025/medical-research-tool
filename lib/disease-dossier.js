...
export async function getDossier(condition, { bypassCache = false } = {}) {
  ...
  if (!bypassCache) {
    ...
    // Share in-flight requests so concurrent callers don't each make
    // their own Claude call. This is the biggest latency win on the
    // gather phase: research.js + evidence.js + trials.js no longer
    // race three parallel 8-10s Claude calls for the same condition.
