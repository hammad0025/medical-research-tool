  ...
  try {
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      ...
      if (r.ok) {
        ...
        if (!parsed) {
          console.warn(`[disease-dossier] could not parse JSON from ${activeModel}. First 500 chars of response: ${text.slice(0, 500)}`);
      ...
      const fallback = isModelNotFoundError(r.status, errJson)
        ...
        : null;
      if (!fallback) {
        console.warn(`[disease-dossier] Claude call failed status=${r.status} model=${activeModel} err=${errTxt.slice(0, 400)}`);
  ...
  try {
    ...
    if (regMatch && regMatch.score >= 55) {
      ...
      if (regDossier) {
        ...
        return { ...regDossier, cacheHit: false, cacheDisabled: !PERSISTENT_CACHE_ENABLED, skippedLlm: true };
      }
    }
  } catch (err) {
    console.warn('[disease-dossier] registry short-circuit failed, falling back to LLM:', err?.message || err);
  }
