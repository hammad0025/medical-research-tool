...
const buildPatientSnapshot = (p = {}) => {
  const rows = [
    ...
    ['Confirmed genetic mutation / variant', p.geneticVariant],
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const r of results) {
      if (r && r.parsed) r.parsed = sanitizeVerdict(r.parsed);
    }
    ...
    if (learnCondition && validation.primary) {
      try {
        const records = verdictToErrorRecords(validation.primary);
