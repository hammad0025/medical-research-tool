...
const VALIDATOR_PROVIDER_TIMEOUT_MS = timeoutFor('validator_provider', 20_000);
...
export const buildPatientSnapshot = (p = {}) => {
  ...
  const rows = [
    ...
    ['Recorded allergies', p.allergies],
    ['Prior treatments / medication history', p.medicationHistory],
    ['Current symptoms', p.symptoms],
    ['Lab work / pulmonary function / studies', p.labWork],
    ['Recent imaging / scans', p.scans],
    ['Pregnancy / breastfeeding status', p.pregnancyStatus],
    ['Kidney / renal function', p.renalFunction],
    ['Liver / hepatic function', p.hepaticFunction],
...
const callPerplexity = async (payload) => {
  ...
  const r = await fetchWithTimeout('https://api.perplexity.ai/chat/completions', {
    ...
    timeoutMs: VALIDATOR_PROVIDER_TIMEOUT_MS,
    provider: 'Perplexity validator'
  });
  if (!r.ok) {
...
export default async function handler(req, res) {
  if (!setSameOriginCors(req, res, { methods: 'POST,OPTIONS', headers: 'Content-Type' })) {
  ...
  try {
    ...
    if (!hasPerplexity && !hasOpenAI && !hasXai) {
      return res.status(503).json({
        error: 'The independent source check is temporarily unavailable.',
    ...
    const unavailable = (provider) => ({
      provider,
      error: 'Independent source check unavailable'
    });
    if (hasPerplexity) jobs.push(callPerplexity(payload).catch(() => unavailable('Perplexity')));
    if (hasOpenAI)     jobs.push(callOpenAI(payload).catch(() => unavailable('OpenAI')));
    if (hasXai)        jobs.push(callXai(payload).catch(() => unavailable('xAI')));
    ...
    if (learnCondition && validation.primary) {
      try {
        ...
        console.warn('[validate.js] error-store persist failed (non-fatal):', safeErrorMessage(e));
      }
    }
    ...
    logError('[validate] request failed', e);
    return res.status(500).json({
      error: 'The independent source check could not be completed.',
