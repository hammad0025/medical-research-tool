...
export default async function handler(req, res) {
  ...
  try {
    ...
    const repurposeSupplementQueries = isRepurpose
      ? [
    ...
    const treatmentQueries = treatments.slice(0, isRepurpose ? 0 : 2).map((t) => `${canonical} ${t}`);
    const queries = dedupeQueries([
      ...synonymSearches.slice(0, isRepurpose ? 1 : 2),
      ...treatmentQueries,
      ...pipelineDrugQueries,
      ...searchSeedQueries,
      ...repurposeSupplementQueries
    ]).slice(0, isRepurpose ? 14 : 8);
    ...
    const webSearchDrugs = (hasStaticKb && !isRepurpose)
      ? []
      : [...drugs, ...pipelineDrugNames, ...searchSeedTerms].slice(0, isRepurpose ? 8 : 6);
    ...
    return res.status(200).json({
      ...
      supplementGatherQueries: isRepurpose ? queries.filter((q) =>
