...
import { admitSourceRows } from './source-admission-gate.js';
import {
...
const isLowIntegrityPublisher = (p) => LOW_INTEGRITY_PUBLISHERS.some((r) => r.test(p || ''));
...
export const normalizeEvidenceFanoutInput = ({
  ...
  // Each source was clamped to 10 results per query, so a condition with a
  // very large literature (IPF has thousands of papers) came back with barely
  // a hundred and the report implied that was all that existed. The prompt
  // pack stays capped, so this widens the pool the ranker chooses from
  // without changing what is sent to the model.
  limitPerSource: Math.max(1, Math.min(40, Number(limitPerSource) || 25))
});

// The drugs we fetch DailyMed labels for: the patient's own medications (needed
// for interaction/safety screening) PLUS the condition's FDA-approved drugs the
// dossier agent named. Previously only patient meds were queried, so an approved
// drug got a real label only if the patient happened to be taking it — leaving
// Approved Treatments cards for uncurated conditions with no verifiable label
// link or safety band. Deduped case-insensitively and capped to bound the
// openFDA fan-out.
export const buildFdaLabelTargets = (patientDrugs = [], dossier = null) => {
  const approved = (dossier?.approvedDrugs || [])
    .map((d) => (typeof d === 'string' ? d : d?.name))
    .filter(Boolean);
  const out = [];
  const seen = new Set();
  for (const drug of [...(patientDrugs || []), ...approved]) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const {
      ...
      limitPerSource: rawLimitPerSource = 25,
    ...
    // Fetch DailyMed labels for the patient's meds AND the condition's approved
    // drugs (from the dossier) so Approved Treatments cards get real labels for
    // any condition, not just ones the patient is already taking.
    const fdaTargetDrugs = buildFdaLabelTargets(drugs, dossier);
    ...
    const [pmRes, epmcRes, oaRes, cochRes, fdaRes, fdaMfgRes, webRes] = await Promise.all([
      ...
      Promise.all(fdaTargetDrugs.map((d) => invoke(openfdaHandler, { drug: d }, req.signal))),
    ...
    const fdaLabels = fdaRes.map((r, i) => ({ drug: fdaTargetDrugs[i], ...(r.body || {}) }));
    const fdaManufacturers = fdaMfgRes.map((r, i) => ({
      manufacturer: manufacturers[i],
      ...(r.body || {})
    }));

    // Retrieval-time admission boundary: only rows with a coherent stable
    // identity, usable source text, non-dossier access, and positive condition
    // relevance may enter either synthesis pool.
    const sourceAdmission = admitSourceRows(cleanPool, {
      condition: canonical || coreQuery,
      aliases: dossier.synonyms || []
    });
    const admittedPool = sourceAdmission.accepted;
    ...
    // The model can only cite what it is handed. The gather now returns hundreds
    // of papers, and capping the pack at 25 meant a report making more than 25
    // sourced statements had claims with nowhere to point — 33 were deleted as
    // unsourced on a single retinitis pigmentosa report.
    const PROMPT_PACK_SIZE = 40;
    const MIN_LIVE_SLOTS = 10;
    const kbRanked = admittedPool.filter((a) => a.isCuratedKB);
    const liveRanked = admittedPool.filter((a) => !a.isCuratedKB);
    ...
    const groundedForPrompt = promptPack.map((a) => {
      ...
      return {
        ...
        conditionSlug: a.conditionSlug || a.kbSlug || null,
        title: a.title,
        journal: a.journal,
        ...
        text: textBlob,
        sourceAdmission: a.sourceAdmission,
        admissionReason: a.admissionReason,
        admissionProvenance: a.admissionProvenance,
        admissionExcerpt: a.admissionExcerpt
      };
    });
    ...
    const qualityBreakdown = {
      ...
      // How many papers the synthesis actually reads and can cite from. Shown
      // alongside totalScreened so neither number can be mistaken for the
      // other: claiming the whole screened pool was "used" overstates exactly
      // as badly as reporting the capped pack as everything that was found.
      citedCount: groundedForPrompt.length,
    ...
    return res.status(200).json({
      ...
      // Curated safety content travels with the pack. Routing it through the
      // dossier instead made it depend on that dossier existing, and a curated
      // file with safety flags but no centres produces no dossier at all —
      // which is 50 of the 61 curated conditions.
      redFlags: kb.meta?.redFlags || [],
      commonComorbidities: kb.meta?.commonComorbidities || [],
      lifestyleCategories: kb.meta?.lifestyleCategories || [],
      ...
      corpusFootprint,
      accessBreakdown,
      sourceAdmissionAudit: sourceAdmission.audit,
      promptPackBreakdown,
      qualityBreakdown,
      knowledgeBase: kb.matched
        ...
        : { matched: false },
      topRanked: admittedPool.slice(0, 50),
      groundedForPrompt,
      fdaLabels,
      fdaManufacturers,
