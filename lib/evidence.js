...
export default async function handler(req, res) {
  ...
  try {
    ...
    // set so pipeline-drug expansion can fire below. Try the raw user string
    // first (so "RP" / "LCA" still hit slug rules), then the dossier canonical
    // and synonyms when the user typed a typo or colloquial name.
    ...
    // (e.g. "IPF metformin") even though they are not pipeline drugs. The KB
    // is a finder, not a static cache of changing literature.
    const searchSeedTerms = (kb.meta?.literatureSearchSeeds || [])
      .map((s) => (typeof s === 'string' ? s : s?.term))
      .filter(Boolean);
    const searchSeedQueries = searchSeedTerms
      .slice(0, 2)
      .map((term) => `${canonical} ${term}`);

    // Full query set: canonical + 1 synonym + treatment cross-products +
    // pipeline-drug + search-seed cross-products. Capped to stay under gather deadline.
    const treatmentQueries = treatments.slice(0, 2).map((t) => `${canonical} ${t}`);
    const queries = [...synonymSearches, ...treatmentQueries, ...pipelineDrugQueries, ...searchSeedQueries];
    ...
    // Perplexity freshness scout — named drugs + pipeline drugs so brand-new
    // approvals / negative trials surface even before they rank in PubMed.
    // Fails open (no key / error / timeout → []), so it never blocks the pack.
    const webSearchDrugs = [...drugs, ...pipelineDrugNames, ...searchSeedTerms].slice(0, 6);
    ...
    const [pmRes, epmcRes, oaRes, cochRes, fdaRes, fdaMfgRes, webRes] = await Promise.all([
      ...
      invoke(perplexitySearchHandler, { condition: canonical, drugs: webSearchDrugs })
    ]);
    ...
    return res.status(200).json({
      ...
      repurposeDrugPool: req.body?.mode === 'repurpose'
        ? await (async () => {
            try {
              const { selectRepurposeDrugs, buildRepurposeDrugScreen } = await import('./drug-registry.js');
              const pool = await selectRepurposeDrugs(dossier, { limit: 24 });
              return pool;
            } catch {
              return [];
            }
        ...
        : [],
      repurposeDrugScreen: req.body?.mode === 'repurpose'
        ? await (async () => {
            try {
              const { selectRepurposeDrugs, buildRepurposeDrugScreen } = await import('./drug-registry.js');
              const pool = await selectRepurposeDrugs(dossier, { limit: 24 });
              return await buildRepurposeDrugScreen(dossier, pool);
            } catch {
              return null;
            }
          })()
