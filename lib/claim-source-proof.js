...
const words = (value) =>
  normalize(value).match(/[a-z][a-z0-9-]{3,}/g)
    ?.filter((word) => !STOP.has(word) && !FUNCTION_WORD.has(word))
    .map(stem) || [];
 ...
 */
const populationSupported = (phrase, sourceNormalized) => {
  if (sourceNormalized.includes(phrase)) return true;
  const tokens = words(phrase);
  if (!tokens.length) return true;
  return tokens.every((token) => sourceNormalized.includes(token));
};
...
export const claimSupportedBySource = (claimText, item, { condition = '' } = {}) => {
  ...
  if (population && !populationSupported(population, sourceNormalized)) {
