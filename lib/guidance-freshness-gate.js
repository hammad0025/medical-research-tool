...
// A publisher routinely redirects a DOI-bearing link onto its own article page
// — journals.lww.com/10.1097/GME…  →  ovid.com/…/10.1097/gme…~article-slug.
// That is the same document in different clothes, not the moved-or-swapped
// target this gate exists to catch, but full-URL comparison called it a
// mismatch and rejected two real society guidelines every night.
// Compare the document identifier when both ends carry one.
// Read the DOI from the PATH only. A search page that carries the DOI as a
// query parameter is a placeholder, not the document, and must not be able to
// impersonate it.
const documentDoi = (url) => {
  let pathname;
  try {
  ...
  const match = pathname.match(/(10\.\d{4,9}\/[^\s?#~]+)/i);
  if (!match) return '';
  return decodeURIComponent(match[1]).toLowerCase().replace(/[.,;)]+$/, '');
};

const redirectPreservesDocument = (url, finalUrl, item) => {
  // A redirect onto a search / redirector page never preserves the document,
  // however much of the identifier it echoes back.
  if (unsafeDocumentUrlReason(finalUrl)) return false;
  const from = documentDoi(url);
  const to = documentDoi(finalUrl);
  if (!from || !to || from !== to) return false;
  // When the item declares its own DOI, the landing page must still be it.
  const declared = String(item?.doi || '')
    .replace(/^https?:\/\/(dx\.)?doi\.org\//i, '').trim().toLowerCase();
  return !declared || declared === to;
};
...
export const assessGuidanceFreshness = (item = {}, {
  ...
  if (
    finalUrl &&
    canonicalIdentity(finalUrl) !== canonicalIdentity(url) &&
    !redirectPreservesDocument(url, finalUrl, item)
  ) {
