...
const diabetesSubtype = (value) => {
  const text = normalize(value)
    .replace(/\btype one\b/g, 'type 1')
    .replace(/\btype two\b/g, 'type 2');
  if (/\b(?:type 1 diabetes|diabetes mellitus type 1|t1d|t1dm)\b/.test(text)) return 'type-1';
  if (/\b(?:type 2 diabetes|diabetes mellitus type 2|t2d|t2dm)\b/.test(text)) return 'type-2';
  return '';
};
...
const conditionMatches = (expectedNames, registryConditions) => {
  ...
  const expectedDiabetesSubtypes = new Set(expected.map(diabetesSubtype).filter(Boolean));
  const actualDiabetesSubtypes = new Set(actual.map(diabetesSubtype).filter(Boolean));
  if (
    expectedDiabetesSubtypes.size === 1 &&
    actualDiabetesSubtypes.size > 0 &&
    ![...actualDiabetesSubtypes].some((subtype) => expectedDiabetesSubtypes.has(subtype))
  ) {
    return false;
  }
  for (const name of expected) {
    if (name.length < 2) continue;
    // Substring containment is only safe for real disease names. A short
    // acronym has to match as a whole word, or "rp" starts accepting any
    // condition that merely spells those letters mid-word.
    if (name.length <= 4) {
      const wholeWord = new RegExp(`\\b${name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i');
      if (actual.some((condition) => wholeWord.test(condition))) return true;
      continue;
    }
    if (actual.some((condition) => condition.includes(name) || name.includes(condition))) {
      return true;
    }
  }
  // Initials matching: "ALS" accepts "amyotrophic lateral sclerosis". Two-letter
  // acronyms are excluded because they collide constantly across unrelated
  // diseases — "RP" matched "Relapsing Polychondritis" here and waved a
  // wrong-disease trial straight past this gate.
  for (const name of expected) {
...
const unsignedTrialPayload = (payload = {}) => ({
  ...
  providerErrors: (Array.isArray(payload?.providerErrors) ? payload.providerErrors : []).map((error) => ({
