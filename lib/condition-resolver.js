...
import { isNarrowerVariant } from './condition-specificity.js';
...
const conditionDefiningConflict = (input, candidate) => {
  ...
  // tuberculosis"), or a prefix-glued sibling ("prediabetes",
  // "silicotuberculosis"). The registry was built from Mondo LEAF nodes with no
  // umbrella/parent entries, so a general term has no correct entry and the
  // fuzzy scorer otherwise grabs an arbitrary child; treating these as different
  // diseases makes the query fall through to the LLM dossier, which researches
  // the general disease correctly. Asymmetric (every caller passes
  // (userInput, matchedEntry)) so a user who typed the specific term still
  // matches a broader entry. Shared with condition-umbrella's child detector.
  if (isNarrowerVariant(a, b)) return true;
  return false;
};
