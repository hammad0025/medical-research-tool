...
import { enqueueBrainRefresh } from './brain-queue.js';
...
async function runBuild(canonical, dossier) {
  ...
  if (!persisted) {
    throw new Error(`dynamic KB for ${slug} did not persist to ${dynamicKbBackendName()}`);
  }
...
export async function ensureDynamicKb(canonical, dossier = {}, options = {}) {
  ...
  const buildP = runBuild(name, dossier)
    .catch(async (err) => {
      console.warn(`[kb-bootstrap] build failed for ${slug}:`, safeErrorMessage(err));
      try {
        await enqueueBrainRefresh(name, slug);
      } catch (queueError) {
        console.warn(`[kb-bootstrap] failed to queue recovery for ${slug}:`, safeErrorMessage(queueError));
      }
      return null;
    })
