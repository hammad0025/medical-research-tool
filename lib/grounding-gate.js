...
const normalizeUrlKey = (u) => String(u || '').trim().toLowerCase().replace(/[).,;]+$/, '').replace(/\/+$/, '');
...
// Does a source mention the patient's condition (any distinctive condition token)?
...
const lookupUrlIndexText = (url, urlIndex) => {
  ...
  let text = urlIndex.get(key);
  if (text == null) {
    for (const [k, v] of urlIndex) {
      if (key.startsWith(k) || k.startsWith(key)) { text = v; break; }
    }
  }
...
export const citationRelevantToSubject = (url, name, urlIndex) => {
  ...
  if (text == null) return null; // unknown source → cannot verify → keep
