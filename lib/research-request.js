...
const splitSynthesisIdempotencyKey = (payload = {}, { profileKey = '' } = {}) => {
...
export const resolvePaidRequestIdempotencyKey = ({
  ...
  // Private-preview requests are authenticated and the server has the full
  // request body. Derive their key from that body so an already-open browser
  // carrying an older client key cannot collide with a previous fresh run.
  if (authenticatedPreview && fingerprint) {
    return `mrt-private-preview-v3:${fingerprint}`;
  }
  return supplied;
};
