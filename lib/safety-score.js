...
export const SAFETY_LEVELS = { Low: 1, Moderate: 2, High: 3 };
export const SAFETY_BANDS = ['Low', 'Moderate', 'High'];
...
const SERIOUS_FAERS_TERMS = [
  'death', 'sudden death', 'cardiac arrest', 'cardiac failure', 'myocardial infarction',
...
export const scoreSafety = ({ drugName, patientMeds, fdaLabel, faers } = {}) => {
  ...
  if (!fdaLabel || !url) return { band: null, level: null, factors: [] };
  ...
  if (seriousHits.length) {
    ...
    const named = seriousHits.slice(0, 3)
      ...
      .join(', ');
    factors.push(cite(`High-frequency serious FAERS post-market reactions: ${named}`));
  }

  // 4. No negative signal → High, with an explicit "why it's high" factor so
  //    even the safest band still carries a clickable FDA source.
  if (!factors.length) {
    factors.push(cite(
...
export const injectSafetyBands = (text, { fdaLabels, patientMeds } = {}) => {
  ...
  for (const line of lines) {
    ...
    if (SAFETY_LINE_RE.test(line)) {
      ...
      if (!scored.band) continue; // no citable evidence → drop the meter
