...
const normalizeTrialProviderErrors = (errors) => {
...
export const trialCollectionDescription = (trials = {}) => {
  ...
  // "of N" must say WHICH N. This counted the delivered pool while the header
  // beside it showed the registry match count, so the same panel reported two
  // different totals with nothing to tell the reader they measure different
  // things.
  return `ClinicalTrials.gov studies with mixed recruitment and access status (${available} of the ${studies.length} studies listed here ${available === 1 ? 'is' : 'are'} currently accepting or available).`;
};
