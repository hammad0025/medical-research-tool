...
export const registryApprovalState = (drug = {}) => {
  if (drug.approvedUsa === true || drug.approvedUSA === true) return 'approved-other-indication';
  if (drug.approvedUsa === false || drug.approvedUSA === false) return 'not-approved';
  const rawFlag = String(drug.approvedUsa ?? drug.approvedUSA ?? '').trim().toLowerCase();
  if (['true', '1', 'yes'].includes(rawFlag)) return 'approved-other-indication';
  if (['false', '0', 'no'].includes(rawFlag)) return 'not-approved';
  const text = String(drug.approvalStatus || drug.status || drug.approvedUsa || '').trim().toLowerCase();
  if (/\bnot approved\b|\bunapproved\b|\binvestigational\b|\bapproval (?:pending|withdrawn|revoked)\b/.test(text)) {
    return 'not-approved';
  }
  if (/\bapproved\b/.test(text)) return 'approved-other-indication';
  return 'unknown';
};
...
const specialtyBoost = (drug, dossier) => {
  ...
  let score = registryApprovalState(drug) === 'approved-other-indication' ? 2 : 0;
...
const loadDrugRegistry = async () => {
  const now = Date.now();
  if (registry && now - loadedAt < TTL_MS) return registry;
  try {
    const raw = await fs.readFile(REGISTRY_PATH, 'utf8');
    registry = parseJsonRejectDuplicates(raw);
    loadedAt = now;
    return registry;
  } catch (e) {
    console.warn('[drug-registry] not loaded:', safeErrorMessage(e));
...
const toPipelineShape = (drug) => ({
  ...
  sponsor: '',
  status: registryApprovalState(drug) === 'approved-other-indication'
    ? 'FDA-approved for at least one US indication; not established for this condition'
    : 'investigational / not US-approved',
  approvalStatus: registryApprovalState(drug),
...
export const selectRepurposeDrugs = async (dossier = {}, { lane = null, limit = 16 } = {}) => {
  const r = await loadDrugRegistry();
  const drugs = (r.drugs || []).filter((d) => d.name && registryApprovalState(d) === 'approved-other-indication');
...
export const buildRepurposeDrugScreen = async (dossier = {}, selectedDrugs = null) => {
  const r = await loadDrugRegistry();
  // Larger pool so Hard-25 registry fill has unused FDA-tracked names with
  // DailyMed-only citations when AI backfill still lands short of 25.
  const pool = selectedDrugs || (await selectRepurposeDrugs(dossier, { limit: 48 }));
  const libraryTotal = r.count || (r.drugs || []).length;
  const approvedTotal =
    r.approvedCount || (r.drugs || []).filter((d) => registryApprovalState(d) === 'approved-other-indication').length;
  return {
    libraryTotal,
    approvedTotal,
    selectedCount: pool.length,
    selectedNames: pool.map((d) => d.name),
    // Lightweight shapes for client-side registry fill (name + mechanism).
    // dailyMedUrl / splSetId are filled later by evidence.js openFDA enrichment
    // so Hard-25 registry fill can attach a real setid label (never a search page).
    fillPool: pool.map((d) => ({
      name: d.name,
      mechanism: d.mechanism || '',
      aliases: d.aliases || [],
      dailyMedUrl: d.dailyMedUrl || '',
      splSetId: d.splSetId || ''
    })),
...
export const buildRepurposeDrugLibraryBlock = (drugs, { lane = null, condition = '' } = {}) => {
  ...
  const lines = drugs.map((d) => {
    ...
    const status = d.approvalStatus === 'approved-other-indication'
      ? 'FDA-approved for another or unspecified indication'
      : 'investigational';
    return `- **${d.name}**${aliases} — ${status}${mech}`;
  });
  return `=== REPURPOSE DRUG LIBRARY${laneLabel} ===
Curated open library of ${drugs.length} FDA-tracked drugs. Inclusion in this list does not establish relevance to ${condition || 'this condition'}; use a drug only when the evidence pack independently supports the condition-specific rationale. Drugs with no human data for THIS condition must be labeled MECHANISTIC_ONLY or PRECLINICAL.
