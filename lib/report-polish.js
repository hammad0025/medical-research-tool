...
// User-facing rewrites for leftover "evidence pack" plumbing (Dorothy hated this).
// Prefer plain-English equivalents over blanking the sentence mid-clause.
const rewriteEvidencePackJargon = (s) =>
  String(s || '')
    .replace(/\b[Nn]o evidence[- ]pack items?\b/g, 'No published source gathered here')
    .replace(/\bevidence[- ]pack items?\b/gi, 'published source')
    .replace(/\bthe evidence[- ]pack\b/gi, 'the sources gathered here')
    .replace(/\bgrounded evidence\b/gi, 'published sources')
...
const NAVIGATIONAL_URL_PATTERNS = [
  ...
  /^https?:\/\/(www\.)?clinicaltrials\.gov\/search\b/i,
  /^https?:\/\/pubmed\.ncbi\.nlm\.nih\.gov\/\?term=/i,
  /^https?:\/\/dailymed\.nlm\.nih\.gov\/dailymed\/drugInfo\.cfm\?setid=/i,
  // FDA drug-lookup (accessdata.fda.gov Drugs@FDA) — the drugname query that
  // lib/openfda.js attaches to every FDA label. It resolves to a registry
  // lookup page (a search by drug name), not a specific citable claim, so it
  // is navigational like DailyMed and must survive link sanitization to keep
  // the evidence-derived safety-band factor links clickable.
...
export const isKnownDeadUrl = (url) =>
  KNOWN_DEAD_URL_RES.some((re) => re.test(String(url || '').trim()));
...
export const stripInvalidMarkdownAnchors = (text) => {
  ...
  // Any remaining [label](bad-href) → plain label (or drop for source ↗).
...
export const sanitizeFabricatedEfficacyScores = (text) => {
  if (!text) return text;
  const replacement =
    'No measured efficacy number in the sources gathered here — ask your doctor or see the linked guideline. Do not treat a made-up percentage as a study result.';
  return String(text)
    .split('\n')
    .map((line) => {
...
export const resolveInlineReferenceMarkers = (text, evidence) => {
  ...
  s = s.replace(/\[#\s*(\d+(?:\s*,\s*#?\d+)*)\s*\]/g, (m, group) => {
    ...
    if (!urls.length) return '';                       // unresolved → drop marker
    return `(${urls.map(inlineCiteAnchor).join(' ')})`;
  });
...
const isClaimSentence = (s) => {
  ...
  if (/^(CANDIDATE|ITEM_KIND|CLASS|APPROVED_FOR|WHAT_IT_DOES|WHY_FOR_THIS_CONDITION|MECHANISM_TARGET|REPURPOSE_RATIONALE|REPURPOSE_SECTION|EVIDENCE_STRENGTH|SUPPORTING_EVIDENCE|REFERENCES|EFFICACY_HYPOTHESIS|PATIENT_SPECIFIC_RISKS|HOW_TO_DISCUSS_WITH_DOCTOR|TREATMENT|SAFETY|CONFIDENCE)\s*:/i.test(t)) return false;
  if (/^(note for this patient|what these terms mean|exports include|disclaimer)\b/i.test(t)) return false;
  if (/^(jump to|tap |sort by|see section|ask your|discuss with your physician before)\b/i.test(t)) return false;
  return true;
};
...
const collectCitationItems = (evidence, trials) => {
  ...
  for (const f of evidence?.fdaLabels || []) {
    ...
    items.push({
      title: String(name),
      name: String(name),
      text: `${name} ${lbl?.indications || ''}`.slice(0, 1200),
      url
    });
  }
  for (const s of trials?.studies || []) {
...
const bestItemForClaim = (sentence, items) => {
...
export const resolveReportConditionSubject = (patient, evidence) => {
  ...
  push(patient?.condition);
  push(evidence?.condition);
  push(evidence?.dossier?.canonical);
  push(evidence?.knowledgeBase?.condition);
  for (const s of patient?.synonyms || []) push(s);
  for (const s of evidence?.dossier?.synonyms || []) push(s);
  for (const s of evidence?.knowledgeBase?.aliases || []) push(s);
...
const conditionOverviewItem = (items, conditionSubject = '', urlIndex = null) => {
  ...
  const pool = conditionSubject && urlIndex
    ? items.filter((it) => citationRelevantToCondition(it.url, conditionSubject, urlIndex) !== false)
    : items;
...
const appendCite = (sentence, url) => {
  const cite = ` (${inlineCiteAnchor(url)})`;
 ...
 *
 * opts.conditionSubject — patient condition (+ synonyms). When set, a pack item
 * whose known text is about a different disease is never attached (Condition
 * Snapshot / research prose had the same wrong-paper class as candidate cards).
 */
export const attachMissingClaimCitations = (text, evidence, trials = null, opts = {}) => {
  ...
  if (!items.length) return { text, attached: 0, stripped: 0 };
  // Subject-relevance guard for CANDIDATE cards. The "link after every claim"
  // pass otherwise attaches whichever pack article shares the most generic
  // disease/mechanism vocabulary (lung, fibrosis, scar, collagen) — so a
  // fibrosis-heavy paper (e.g. the metformin Nat-Med study) gets slapped onto a
  // Losartan / Vitamin D / Rapamycin card whose drug the source never mentions.
  // Inside a candidate block we therefore attach a source ONLY when it is not
  // positively off-topic for THAT card's drug; an off-topic best-match is left
  // unlinked (honest) rather than mis-cited or deleted.
  //
  // Condition gate (research prose + cards): same three-valued contract for the
  // patient's disease — a sickle-cell review must not cite an RP snapshot claim
  // even when token-overlap / overview ranking prefers it.
  ...
  const out = lines.map((line) => {
    const trimmed = line.trim();
    if (!trimmed) return line;
    { const cm = trimmed.match(/^CANDIDATE:\s*(.+)$/i); if (cm) currentSubject = cm[1].replace(/\*/g, '').split(/[—–\-:|]/)[0].trim(); }
    if (/^\s{0,3}#{1,6}\s/.test(line)) return line;
    if (/^CANDIDATE:/im.test(trimmed)) return line;
    ...
    for (const sent of sentences) {
      ...
      const { item, score } = bestItemForClaim(st, conditionEligibleItems);
      // Inside a candidate card, a source we can POSITIVELY show does not
      // mention this card's drug is off-topic — do not attach it. (null/true =
      // unknown-or-relevant → allowed, so we never strip verifiable citations.)
      ...
      const conditionRelevant = (url) =>
        !conditionSubject || citationRelevantToCondition(url, conditionSubject, urlIndex) !== false;
      const mayAttach = (url) => subjectRelevant(url) && conditionRelevant(url);
      // Primary: real token overlap with a pack item.
      if (item && score >= 0.10 && mayAttach(item.url)) {
        attached += 1;
        rebuilt.push(appendCite(sent, item.url));
        continue;
      }
      // General condition definition / path: attach the best overview source.
      if (isGeneralConditionClaim(st) && overview?.url && mayAttach(overview.url)) {
        attached += 1;
        rebuilt.push(appendCite(sent, overview.url));
        continue;
      }
      // Must-cite with a real (non-trivial) match — still attach.
      if (isMustCite(st) && item && score >= 0.14 && mayAttach(item.url)) {
        attached += 1;
        rebuilt.push(appendCite(sent, item.url));
        continue;
      }
      // Must-cite / hard claim with nothing usable → strip (no unsourced facts).
      // EXCEPTION: inside a candidate card we KEEP the mechanistic rationale as
      // honest unlinked text (repurposing hypotheses are the whole point of the
      // card, framed by EVIDENCE_STRENGTH / SUPPORTING_EVIDENCE) rather than
      // deleting it or wrongly citing an off-topic drug's paper.
      if (!currentSubject && (isMustCite(st) || (isHardClaim(st) && score < 0.05))) {
        stripped += 1;
        continue;
      }
      rebuilt.push(sent);
...
// ===========================================================================
const CANDIDATE_SUBJECT_RE = /^CANDIDATE:\s*(.+)$/im;
const CANDIDATE_CITELINE_RE = /^(REFERENCES|SUPPORTING_EVIDENCE|SUPPORTING EVIDENCE)\s*:/i;

export const enforceCandidateCitationRelevance = (text, evidence) => {
  if (!text || !/^CANDIDATE:/im.test(String(text))) return { text, demoted: [] };
  const urlIndex = buildEvidenceUrlIndex(evidence);
  if (!urlIndex.size) return { text, demoted: [] };
  const demoted = [];
  const blocks = String(text).split(/(?=^CANDIDATE:\s)/gim);
  const out = blocks
    .map((block) => {
      const subj = (block.match(CANDIDATE_SUBJECT_RE) || [])[1];
      ...
      return block
        ...
        .map((line) => {
          ...
          return line.replace(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g, (m, label, url) => {
            const rel = citationRelevantToSubject(url, subject, urlIndex);
            if (rel === false) {
...
export const enforceConditionCitationRelevance = (text, evidence, patient = null) => {
  ...
  if (!urlIndex.size) return { text, demoted: [] };
  const demoted = [];
  const out = String(text).replace(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g, (m, label, url) => {
    const rel = citationRelevantToCondition(url, conditionSubject, urlIndex);
    if (rel === false) {
      demoted.push(`${conditionSubject}: ${url}`);
      // Generic polish anchors have no reader value without a URL; drop them.
      // Distinctive model labels (paper titles) stay as plain text.
      if (/^source\s*↗?$/i.test(String(label).trim())) return '';
      return label;
    }
    return m;
...
export const finalizeReportText = (text, { evidence, trials, validation, evidenceGrade, patient } = {}) => {
  ...
  // Link bare NCT IDs to ClinicalTrials.gov study pages (specific records).
  ...
  out = sanitizeMarkdownLinks(out, allowedUrls);
  // Task B: demote card citations whose source text does not mention the card's
  // drug (off-topic-but-live link). Deterministic; runs before + independent of
  // the validator so it holds even when the second AI errors.
  const relevance = enforceCandidateCitationRelevance(out, evidence);
  if (relevance.demoted.length) {
    console.warn(
      `[report-polish] demoted ${relevance.demoted.length} off-topic card citation(s) ` +
      `(source does not mention the card's subject): ${[...new Set(relevance.demoted)].slice(0, 12).join(' | ')}`
    );
  }
  out = relevance.text;
  ...
  // After validator fixes may re-introduce patterns, re-run efficacy + Google-as-paper.
  out = sanitizeFabricatedEfficacyScores(out);
  out = demoteGoogleAsPaperCitations(out);
  ...
  // Final guarantee: no DailyMed search page ever reaches the reader as a link,
  // even if one was authored after the earlier pass (validator fix / reattach).
  return stripDailyMedSearchLinks(relinked.text);
};
