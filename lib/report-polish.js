...
import { enforceTrialRegistryNarrative } from './trial-registry-gate.js';
import {
...
export const attachMissingClaimCitations = (text, evidence, trials = null, opts = {}) => {
  ...
  const out = lines.map((line, lineIndex) => {
    const trimmed = line.trim();
    if (!trimmed) return line;
    { const cm = trimmed.match(/^(?:CANDIDATE|Drug or supplement idea):\s*(.+)$/i); if (cm) currentSubject = cm[1].replace(/\*/g, '').split(/[—–\-:|]/)[0].trim(); }
    if (/^\s{0,3}#{1,6}\s/.test(line)) return line;
    if (/^(?:CANDIDATE|Drug or supplement idea):/im.test(trimmed)) return line;
    ...
    for (const sent of sentences) {
      ...
      if (isMustCite(st) || currentSubject || (isHardClaim(st) && score < 0.05)) {
        stripped += 1;
        continue;
      }
      rebuilt.push(sent);
