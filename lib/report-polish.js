...
import { sanitizePublicText, fieldLabelSpellings, READER_FIELD_LABELS } from './public-language.js';
...
export const replaceLifestyleSubsection = (text, dossier, evidence) => {
  ...
  const pattern = /(^|\n)(Non-Drug\s*\/?\s*Lifestyle[^\n]*)\n[\s\S]*?(?=\n(?:Patient Advocacy|Insurance|Drug-Drug|##\s*\d)|$)/i;
  if (!pattern.test(source)) return source;
  const lines = [];
  for (const rec of recommendations.slice(0, 8)) {
    lines.push(`- ${String(rec.recommendation).replace(/\s+/g, ' ').trim()}`);
  }
...
export const finalizeReportText = (text, { evidence, trials, validation, evidenceGrade, patient } = {}) => {
  ...
  sealed = replaceClinicalTrialsSection(sealed, trials);
  const curatedDossier = evidence?.dossier || patient?.dossier || null;
  sealed = replaceCentersSection(sealed, curatedDossier);
  sealed = replaceAdvocacySubsection(sealed, curatedDossier);
  sealed = replaceLifestyleSubsection(sealed, curatedDossier, evidence);
  sealed = replaceAdvancedTherapiesSection(sealed, evidence, curatedDossier);
