...
import { clampText, isUnverifiedRegulatoryStatus } from './card-identity.js';
import {
...
export const citationAnchorLabel = (item, url) => {
  ...
  const raw = rewriteMarkdownLinks(authored, (_match, label) => label)
    ...
    .replace(/\s+/g, ' ')
    .trim();
  // One shared clamp (lib/card-identity.js) instead of a local slice. The
  // marker is safe again now that splitClaimSentences treats a markdown link
  // as atomic, so a shortened title is once more visibly shortened rather
  // than silently passing for a complete one.
  if (raw) return clampText(raw, 96);
  try {
...
const splitClaimSentences = (line) => {
  ...
  // A markdown link is atomic. A shortened citation label ending in "…" —
  // which sanitizePublicText NFKC-rewrites into three ASCII periods — used to
  // split the link down the middle. The half without the URL then read as an
  // unsourced claim and was either re-cited or deleted, which is how one
  // title came to render twice with unbalanced parentheses. Protecting the
  // link here is also what lets a label carry a truncation marker at all.
  const linkRanges = [];
  for (const m of s.matchAll(/\[[^\]]*\]\([^)]*\)/g)) {
    linkRanges.push([m.index, m.index + m[0].length]);
  }
  const insideLink = (i) => linkRanges.some(([from, to]) => i >= from && i < to);
  const parts = [];
  let buf = '';
...
/**
 * Section 3 says "Approved Treatments". Only approved treatments may stand in
 * it.
 *
 * Membership was whatever the model wrote — there was no gate at all — and the
 * prompt explicitly permits investigational agents "if genuinely important".
 * One report shipped MCO-010, jCell and NACA under that heading, each reading
 * "FDA status: Unknown — insufficient verified FDA/DailyMed label evidence",
 * which is the label gate's own words for "we could not verify this".
 *
 * The test is isUnverifiedRegulatoryStatus, not "is it approved": an
 * off-label card names a drug with a real FDA label for another indication,
 * which this product deliberately shows with a flag. What must not stand here
 * is a card whose regulatory standing was never established at all.
 * Runs while labels are still in raw schema form, before the reader-label
 * rewrite, and only inside section 3 — these agents are not being deleted from
 * the report, they belong to (and already appear in) the researched section.
 */
export const dropUnapprovedFromApprovedSection = (text) => {
  const source = String(text || '');
  const section = /(^|\n)(##\s*3\.[^\n]*\n)([\s\S]*?)(?=\n##\s*\d|$)/i;
  const match = source.match(section);
  if (!match) return source;
  const body = match[3];
  if (!CARD_SUBJECT_RE.test(body)) return source;
  const blocks = body.split(CARD_SUBJECT_SPLIT_RE);
  const kept = blocks.filter((block, index) => {
    // blocks[0] is whatever preceded the first card — section prose, keep it.
    if (index === 0 && !CARD_SUBJECT_RE.test(block)) return true;
    const status = block.match(/^(?:FDA[_\s-]?STATUS|APPROVAL_STATUS|FDA status)\s*:\s*(.+)$/im);
    // A card with no status line at all is left alone: absent metadata is a
    // model omission, and deleting a genuine approved drug over it would be a
    // worse failure than the one this guards against.
    if (!status) return true;
    return !isUnverifiedRegulatoryStatus(status[1]);
  });
  if (kept.length === blocks.length) return source;
  return source.replace(section, (full, lead, heading) => `${lead}${heading}${kept.join('')}`);
};
...
export const replaceClinicalTrialsSection = (text, trials) => {
  ...
  // `studies` is the pool delivered to this renderer, and that pool is itself
  // capped in transport (trimTrialPool). Using its length as the match count
  // under-reported the search and disagreed with the trials table's own
  // header on the same page. The registry's post-filter total is the honest
  // denominator for "matching records".
  const matched = Number(trials?.total ?? trials?.returned) || studies.length;
  const truncationNote = matched > shown.length
    ? `Showing the first ${shown.length} of ${matched} matching records; the full list is in the clinical trials table.`
    : '';
...
export const finalizeReportText = (text, {
  ...
  sealed = tracePass('dropUnapprovedFromApprovedSection', sealed, dropUnapprovedFromApprovedSection(sealed));
  ...
  // The curated lists sit at evidence.* on the synthesis path and at
  // evidence.knowledgeBase.* on the polish path. These fallbacks read only
  // the first, so on the polish path they resolved to nothing and the
  // dossier was the sole remaining source — fine for a condition whose
  // dossier happens to carry the field, silently empty for one whose
  // knowledge base is the only place it exists.
  const curatedList = (field) => {
    const direct = evidence?.[field];
    if (Array.isArray(direct) && direct.length) return direct;
    const nested = evidence?.knowledgeBase?.[field];
    return Array.isArray(nested) ? nested : [];
  };
  const lifestyleEvidence = {
    ...(evidence || {}),
    lifestyleRecommendations: curatedList('lifestyleRecommendations'),
    lifestyleCategories: curatedList('lifestyleCategories')
  };
  sealed = tracePass('replaceLifestyleSubsection', sealed, replaceLifestyleSubsection(sealed, curatedDossier, lifestyleEvidence));
  ...
  sealed = replaceCuratedSafetySection(sealed, {
    ...(curatedDossier || {}),
    redFlags: curatedDossier?.redFlags?.length ? curatedDossier.redFlags : curatedList('redFlags'),
    commonComorbidities: curatedDossier?.commonComorbidities?.length
      ? curatedDossier.commonComorbidities
      : curatedList('commonComorbidities')
  });
