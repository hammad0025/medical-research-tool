  ...
  if (primary) {
    ...
    for (const d of primary.disputed || []) {
      if (d?.correction) {
        ...
        // No correction, but the validator disputes the claim. If the disputed
        // sentence carries an inline citation, that link is asserting support
        // the second AI says isn't there (Dorothy: "these links don't have
        // anything to do with what the claim is"). Demote the link on the
        // matching line to plain text — keep the prose (the dispute is still
        // surfaced as a badge), just remove the misleading clickable citation.
        out = demoteLinksOnMatchingLine(out, d.quote || d.claim);
      }
    }
