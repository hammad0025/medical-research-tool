...
const isClaimSentence = (s) => {
  ...
  if (/^(CANDIDATE|CLASS|APPROVED_FOR|WHAT_IT_DOES|WHY_FOR_THIS_CONDITION|MECHANISM_TARGET|REPURPOSE_RATIONALE|EVIDENCE_STRENGTH|SUPPORTING_EVIDENCE|REFERENCES|EFFICACY_HYPOTHESIS|PATIENT_SPECIFIC_RISKS|HOW_TO_DISCUSS_WITH_DOCTOR|TREATMENT|SAFETY|CONFIDENCE)\s*:/i.test(t)) return false;
  ...
  const out = blocks
    .map((block) => {
      ...
      return block
        .split('\n')
        .map((line) => {
          if (!CANDIDATE_CITELINE_RE.test(line.trim())) return line;
