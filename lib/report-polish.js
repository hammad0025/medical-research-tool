...
// search pages and DailyMed drug lookups. Specific PubMed PMIDs, DOIs, PMC
...
const fallbackEntityUrl = (name) => {
  ...
  // Prefer DailyMed for drug-like names (letters / hyphens / spaces) so a
  // demoted citation is never replaced with Google-as-paper. Google remains
  // only for clearly non-drug entities (centers/orgs) via Search ↗ labeling.
  if (/^[A-Za-z][A-Za-z0-9 .'-]{1,60}$/.test(t) && !/\b(clinic|hospital|university|institute|foundation|society|center|centre)\b/i.test(t)) {
    return `https://dailymed.nlm.nih.gov/dailymed/search.cfm?query=${encodeURIComponent(t)}`;
  }
