...
// Client mandate — "real specific URL or none": we only synthesize a link when
// it points to a SPECIFIC record (an NCT study). We NEVER fabricate a DailyMed
// or Google SEARCH link here (a search page is not a citation and, per the
// reviewer, a fake/empty link is worse than plain text). Anything else returns
// '' so reattachEntityLinks leaves the entity as plain (bold) text.
...
export const reattachEntityLinks = (text) => {
  ...
  const out = lines.map((line) => {
    ...
    return line.replace(BOLD_SPAN_RE, (m, a, b) => {
      ...
      // "Real specific URL or none": when we cannot resolve a SPECIFIC record
      // (only NCT studies qualify), leave the entity as plain bold text rather
      // than fabricate a search link.
      if (!url) return m;
...
export const finalizeReportText = (text, { evidence, trials, validation, evidenceGrade, patient } = {}) => {
  ...
  // Prefer DailyMed/NCT when we can; Google fallback is for named entities only
  // and never counts as a Hard-25 paper citation.
