...
/** @deprecated use rewritePatientJargon */
const rewriteEvidencePackJargon = rewritePatientJargon;
...
const isClaimSentence = (s) => {
  ...
  if (/^(CANDIDATE|ITEM_KIND|CLASS|REPURPOSE_SECTION|EVIDENCE_STRENGTH|SUPPORTING_EVIDENCE|REFERENCES|HOW_TO_DISCUSS_WITH_DOCTOR|TREATMENT|CONFIDENCE)\s*:/i.test(t)) return false;
...
const hasQuantitativeClaim = (s) =>
  /(?:\b\d+(?:[.,]\d+)?\s*(?:%|mg|mcg|µg|g|kg|mL|ml|L|units?|patients?|people|years?|months?|weeks?|days?)\b|\bNCT\d{8}\b)/i
    .test(String(s || ''));

const citationItemSupportsClaim = (sentence, item) => {
  if (!hasQuantitativeClaim(sentence)) return true;
  const claim = String(sentence || '')
    .replace(/^\s*[-*•]\s*/, '')
    .replace(/^\s*[A-Z][A-Z0-9_ -]{2,}:\s*/, '')
    .replace(/\[[^\]]+\]\(https?:\/\/[^)]+\)/g, '');
  return isClaimGrounded(
    claim,
    buildGroundingIndex({ evidencePack: [item] })
  );
};
...
const bestItemForClaim = (sentence, items) => {
  ...
  for (const it of items) {
    if (!citationItemSupportsClaim(sentence, it)) continue;
 ...
 */
export const enforceQuantitativeCitationSupport = (text, evidence, trials = null) => {
  ...
  const out = String(text).split('\n').map((line) => {
    if (!hasQuantitativeClaim(line) || isMarkdownTableRow(line)) return line;
    return splitClaimSentences(line).map((sentence) => {
      if (!hasQuantitativeClaim(sentence) || !sentenceHasLink(sentence)) return sentence;
      let supported = 0;
      const rewritten = rewriteMarkdownLinks(sentence, (match, label, href) => {
        const item = byUrl.get(canonicalizeCitationUrl(href));
        if (item && citationItemSupportsClaim(sentence, item)) {
          supported += 1;
          return match;
        }
        demoted += 1;
  ...
  const out = lines.map((line) => {
    ...
    if (isMarkdownTableRow(line)) {
      ...
      for (let i = start; i < end; i++) {
        ...
        if (!cell || sentenceHasLink(cell) || !isClaimSentence(cell) || !isMustCite(cell)) continue;
        const { item, score } = bestItemForClaim(cell, conditionEligibleItems);
        const relevant = item && score >= 0.10
      ...
      return `${line.match(/^\s*/)?.[0] || ''}${cells.join('|')}`;
    }
...
export const finalizeReportText = (text, { evidence, trials, validation, evidenceGrade, patient } = {}) => {
  ...
  const quantitativeCites = enforceQuantitativeCitationSupport(out, evidence, trials);
  ...
  sealed = enforceQuantitativeCitationSupport(sealed, evidence, trials).text;
  {
  ...
  return sanitizePublicText(sealed);
};
