...
export const citationAnchorLabel = (item, url) => {
  ...
  const raw = rewriteMarkdownLinks(authored, (_match, label) => label)
    ...
    .replace(/\s+/g, ' ')
    .trim();
  // Deliberately no ellipsis. sanitizePublicText NFKC-normalizes "…" into
  // three ASCII periods, and this label sits INSIDE a markdown link — so the
  // claim-sentence splitter then saw ". " mid-link, cut the link in half,
  // failed to find a URL in the first half, and re-cited it. That is what
  // produced titles rendered twice with unbalanced parentheses. Cutting on a
  // word boundary with no terminal punctuation keeps the label idempotent.
  if (raw) {
    if (raw.length <= 96) return raw;
    const cut = raw.slice(0, 93);
