...
export const stripInvalidMarkdownAnchors = (text) => {
  ...
  // Known-dead hosts (offline RetNet) and other bad hrefs → plain label / drop source ↗.
...
const appendCite = (sentence, url) => {
  const anchor = inlineCiteAnchor(url);
...
export const attachMissingClaimCitations = (text, evidence, trials = null, opts = {}) => {
  ...
  const out = lines.map((line) => {
    ...
    // NEVER inject "(source ↗)" into GFM tables — smashes Pipeline Watch into
    // literal pipes / header junk and leaves orphan ")" on linked drug cells.
    if (isMarkdownTableRow(line)) return line;
...
export const finalizeReportText = (text, { evidence, trials, validation, evidenceGrade, patient } = {}) => {
  ...
  // Skip GFM table rows so we never corrupt Pipeline Watch cell markup.
  out = String(out).split('\n').map((line) =>
    isMarkdownTableRow(line) ? line : linkBareNctIds(line)
  ).join('\n');
