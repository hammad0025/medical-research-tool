...
export const preferVerifiableUrl = (a, fallback = '') => {
  ...
  for (let i = 1; i < cands.length; i++) {
    const r = verifiableUrlRank(cands[i]);
    if (r < bestRank) { best = cands[i]; bestRank = r; }
  }
  return best;
};
...
const inlineCiteAnchor = (url) => `[source ↗](${url})`;
...
const conditionOverviewItem = (items) => {
  ...
  const ranked = [...items].sort((a, b) => {
    const score = (it) => {
      ...
      if (/parkinson|fibrosis|retinitis|disease|disorder/.test(blob)) s += 1;
 ...
 */
export const attachMissingClaimCitations = (text, evidence, trials = null) => {
  ...
  const overview = conditionOverviewItem(items);
  ...
  const out = lines.map((line) => {
    ...
    for (const sent of sentences) {
      ...
      const { item, score } = bestItemForClaim(st, items);
...
export const finalizeReportText = (text, { evidence, trials, validation, evidenceGrade, patient } = {}) => {
  ...
  const claimCite = attachMissingClaimCitations(out, evidence, trials);
