...
// spironolactone label under "Usually used for", putting one drug's indication
// and source on another drug.
const CANDIDATE_SELF_FIELDS = new Set([
  'PATIENT_SPECIFIC_RISKS', 'SAFETY', 'CONFIDENCE', 'EFFICACY_HYPOTHESIS',
  'HOW_TO_DISCUSS_WITH_DOCTOR', 'APPROVED_FOR', 'WHAT_IT_DOES', 'CLASS'
]);
...
export const finalizeReportText = (text, { evidence, trials, validation, evidenceGrade, patient } = {}) => {
  ...
  sealed = stripOrphanCardFragments(sealed);
  // Last: remove the structure left standing over content the passes above
  // removed, so the reader never sees a heading or a column header promising
