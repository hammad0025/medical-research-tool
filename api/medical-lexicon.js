...
// Score boost when trials touch premier centers. Small per-match (so a trial
// with 5 top centers doesn't 5x a trial with 1); capped at 15.
export const topCenterBoost = (texts) => {
  const arr = (Array.isArray(texts) ? texts : [texts]).filter(Boolean);
  let hits = 0;
  for (const t of arr) if (isTopCenter(t)) hits++;
