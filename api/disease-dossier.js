...
const DOSSIER_MODEL = process.env.ANTHROPIC_DOSSIER_MODEL || 'claude-3-5-haiku-20241022';
...
Return the JSON dossier now.`;
  ...
  try {
    ...
    if (!r.ok) {
      ...
      console.warn('[disease-dossier] Claude call failed', r.status, err);
      return null;
    }
    ...
    if (!parsed) {
      console.warn('[disease-dossier] could not parse JSON from Haiku response');
      return null;
    }
    return parsed;
  } catch (e) {
    console.warn('[disease-dossier] fetch threw:', e.message);
    return null;
  }
}
...
export async function getDossier(condition, { bypassCache = false } = {}) {
  ...
  const dossier = raw ? sanitizeDossier(raw, input) : makeFallback(input);
